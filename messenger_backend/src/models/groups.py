...
class Groups(UUIDPrimaryKey, Base):
    ...
    # Отношения
    creater_user: Mapped["Users"] = relationship(foreign_keys=[created_by], back_populates="create_groups") # Создатель группы
    group_members: Mapped[list["GroupMembers"]] = relationship(foreign_keys="GroupMembers.group_id", back_populates="group") # Участники группы
    last_message: Mapped["Messages"] = relationship(foreign_keys=[last_message_id])
    messages: Mapped[list["Messages"]] = relationship(foreign_keys="Messages.group_id", back_populates="group", order_by="Messages.created_at")
