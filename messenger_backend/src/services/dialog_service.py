...
class DialogService:
    ...
    async def check_user_is_dialog_interlocutor(
        ...
        """Проверка, что пользователь является одним из собеседников диалога"""
    ...
    @staticmethod
    async def get_dialog_interlocutors_username(
        dialog_id: UUID, session: AsyncSession
    ) -> set[str]:
        """Получение username-участников диалога"""
        dialog = await DialogCRUD.get_dialog_by_id_with_relationships(
            dialog_id, session
        )
        return {dialog.user1.username, dialog.user2.username}

    @staticmethod
    async def get_dialogs_by_user(
        user: Users, session: AsyncSession
    ) -> list[ChatResponse]:
        ...
        for dialog in dialogs:
            unread_count = await MessageCRUD.unread_count_message_by_chat(
                dialog_id=dialog.id, user_id=user.id, session=session
            )
            interlocutor = (
                dialog.user2 if dialog.user1_id == user.id else dialog.user1
            )  # TODO: свое именование (контакт)
            chat = ChatResponse(
                id=dialog.id,
    ...
    async def create_dialog(
        creator: Users,
        dialog_data: CreateDialogRequest,
        session: AsyncSession,
    ) -> Dialogs:
        """Создание диалога"""
        ...
        await UserService.get_user_by_id(
            user_id=dialog_data.interlocutor, session=session
        )
        # проверка на уже имеющийся диалог
        if (
            await DialogCRUD.get_dialog_between_users(
                user1_id=creator.id, user2_id=dialog_data.interlocutor, session=session
            )
            is not None
        ):
        ...
        return await DialogCRUD.get_dialog_by_id_with_relationships(
            new_dialog.id, session
        )
